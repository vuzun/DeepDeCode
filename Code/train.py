...
class Training():
    def __init__(self, config,
        ...
        self.encoded_seq = np.loadtxt(data_path + '/' + encoded_seq_filename) 
        self.y_label_filename=y_label_filename
        
        
        if self.config['MODEL_NAME']=="AttLSTM":
            no_timesteps = int(len(self.encoded_seq[0]) / self.config['MODEL']['embedding_dim'])
            att_DNA_args = {'n_nts': config['MODEL']['embedding_dim'], 'n_bins': no_timesteps, # 4 x 100 nt
        ...
        elif self.config['MODEL_NAME']=='SimpleLSTM':
            ...
            self.model = eval(config['MODEL_NAME'])(config['MODEL']['output_dim'], self.device, config['DATA']['BATCH_SIZE'],
                                2, config['MODEL']["filter_num"], config['MODEL']["kernel_size"], config['MODEL']["conv_layer_stride"], 
                                config['MODEL']["max_pool_kernel_size"], config['MODEL']["max_pool_stride"], 
                                None, config['MODEL']['fc_neuron_num'], 
                                config['TRAINER']['dropout'])
        elif self.config['MODEL_NAME']=='SimpleLinear':
            self.model = eval(config['MODEL_NAME'])(config['MODEL']['embedding_dim']*config['MODEL']['subsequence_length'],
                config['MODEL']['hidden_dim_linear'], config['MODEL']['output_dim'])
    ...
    def train_one_epoch(self, epoch, x_train, y_train):
        ...
        for bnum, sample in tqdm(enumerate(self.trainloader)):
            ...
            # attention models have an additional attention map output
            if self.config['MODEL_NAME']=="AttLSTM":
    ...
    def val_one_epoch(self, epoch, x_val, y_val):
        ...
        valset = SequenceDataset(x_val, y_val)
    ...
    def training_pipeline(self):
        ...
        if not self.train_from_checkpoint:
            ...
            no_timesteps = int(len(self.encoded_seq[0]) / self.config['MODEL']['embedding_dim'])
            encoded_seq = self.encoded_seq.reshape(-1, no_timesteps, self.config['MODEL']['embedding_dim'])
...
if __name__ == "__main__":
    ...
    sys_params_file_name="system_specific_params.yaml"
    chrm = "all/"

    if len(sys.argv)>1:
        config_file_name=sys.argv[1]
    if len(sys.argv)>2:
        sys_params_file_name=sys.argv[2]
    ...
    with open(sys_params_file_name, 'r') as params_file:
    ...
    obj = Training(config, model_name_save_dir, data_path, model_dir, tb_path, att_path)
